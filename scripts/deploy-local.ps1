...
foreach ($AddonName in $AddonsToDeploy) {
    ...
    try {
        ...
        Write-Success "  [OK] Deployed successfully to: $TargetPath"
        $SuccessCount++
    }
    catch {
        Write-Error "  [FAIL] Failed to deploy: $_"
        $FailCount++
    }
}
...
Write-Info "    .\scripts\deploy-local.ps1 -AddonsToDeploy @('plugin.video.seranov.recursive', 'context.seranov.screenshots')"
Write-Info ""
