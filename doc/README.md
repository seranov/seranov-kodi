# Documentation / Документация

This folder contains all project documentation in English and Russian.

Эта папка содержит всю проектную документацию на английском и русском языках.

---

## 📖 Main Documentation / Основная документация

### English

- **[INSTALLATION.md](INSTALLATION.md)** - Complete installation guide
  - Installing repository add-on
  - Adding repository source
  - Troubleshooting

- **[QUICKSTART.md](QUICKSTART.md)** - Quick start guide
  - Fast installation steps
  - Basic configuration
  - Common usage scenarios

- **[CONTRIBUTING.md](CONTRIBUTING.md)** - Developer's guide
  - Creating new add-ons
  - Service plugin templates
  - Build and deployment scripts

### Русский

- **[INSTALLATION.ru.md](INSTALLATION.ru.md)** - Полное руководство по установке
  - Установка репозитория
  - Добавление источника репозитория
  - Устранение неполадок

- **[QUICKSTART.ru.md](QUICKSTART.ru.md)** - Краткое руководство
  - Быстрая установка
  - Базовая настройка
  - Типичные сценарии использования

- **[CONTRIBUTING.ru.md](CONTRIBUTING.ru.md)** - Руководство для разработчиков
  - Создание новых аддонов
  - Шаблоны плагинов-служб
  - Скрипты сборки и развертывания

- **[README.ru.md](README.ru.md)** - Основное README на русском языке

---

## 📝 Release Documentation / Документация релизов

### English

- **[RELEASE_NOTES.md](RELEASE_NOTES.md)** - Release notes and changelog
- **[RELEASE_GUIDE.md](RELEASE_GUIDE.md)** - Quick release guide
- **[BUILD_AND_PUBLISH.md](BUILD_AND_PUBLISH.md)** - Complete build and deployment guide
  - Local build instructions
  - GitHub Actions workflow
  - GitHub Pages deployment
  - Troubleshooting

### Русский

- **[BUILD_AND_PUBLISH.ru.md](BUILD_AND_PUBLISH.ru.md)** - Полное руководство по сборке и публикации
  - Локальная сборка
  - GitHub Actions workflow
  - Развертывание на GitHub Pages
  - Устранение неполадок
- **[RELEASE_GUIDE.md](RELEASE_GUIDE.md)** - How to create a new release
- **[RELEASE_CHECKLIST.md](RELEASE_CHECKLIST.md)** - Pre-release checklist
- **[RELEASE_COMPLETE.md](RELEASE_COMPLETE.md)** - Post-release procedures

---

## 🔗 Quick Links / Быстрые ссылки

### For Users / Для пользователей

- 🚀 [Quick Start (EN)](QUICKSTART.md) | [Быстрый старт (RU)](QUICKSTART.ru.md)
- 📦 [Installation (EN)](INSTALLATION.md) | [Установка (RU)](INSTALLATION.ru.md)
- 💡 **Repository URL:** `https://raw.githubusercontent.com/seranov/seranov-kodi/main/repo/`

### For Developers / Для разработчиков

- 🛠️ [Contributing Guide (EN)](CONTRIBUTING.md) | [Руководство (RU)](CONTRIBUTING.ru.md)
- 📜 [Scripts Documentation](../scripts/README.md)
- 🔖 [Release Guide](RELEASE_GUIDE.md)

---

## 🌐 Repository Information / Информация о репозитории

- **GitHub:** https://github.com/seranov/seranov-kodi
- **Issues:** https://github.com/seranov/seranov-kodi/issues
- **License:** MIT
- **Author:** seranov (seranov@yandex.ru)

---

## 📚 Additional Resources / Дополнительные ресурсы

### Official Kodi Documentation / Официальная документация Kodi

- [Add-on Development](https://kodi.wiki/view/Add-on_development)
- [Python API](https://codedocs.xyz/xbmc/xbmc/)
- [JSON-RPC API](https://kodi.wiki/view/JSON-RPC_API)

### Project Structure / Структура проекта

```
doc/
├── README.md                    # This file / Этот файл
├── index.md                     # Legacy index (deprecated)
│
├── INSTALLATION.md              # Installation guide (EN)
├── INSTALLATION.ru.md           # Руководство по установке (RU)
│
├── QUICKSTART.md                # Quick start (EN)
├── QUICKSTART.ru.md             # Быстрый старт (RU)
│
├── CONTRIBUTING.md              # Developer's guide (EN)
├── CONTRIBUTING.ru.md           # Руководство для разработчиков (RU)
│
├── README.ru.md                 # Main README in Russian
│
├── RELEASE_NOTES.md             # Release notes
├── RELEASE_GUIDE.md             # Release creation guide
├── RELEASE_CHECKLIST.md         # Release checklist
└── RELEASE_COMPLETE.md          # Post-release procedures
```

